...
import { calcularNuevoSaldo, calculateCartera, mapCarteraResults } from "../utils/funtions";
...
const getDispatchValidation = ({
  ...
  if (!documento) {
    return {
      ...
      validationReason: "Sin documento de vendedor",
    };
  }
  ...
  if (!phone) {
    return {
      isValidForDispatch: false,
      validationReason: "Sin contacto válido en INFOCONTACTO",
    };
  }
...
const buildPortfolioSummary = async ({
  ...
  if (!Number.isInteger(vinculado) || vinculado <= 0) {
    throw new Error(
      `Vinculado inválido: ${vinculado}. Debe ser un número positivo.`,
    );
  }
  ...
  const SQL_CODES =
    SellerPowerBi?.CCOSTO === "39632" ? CODIGOS_SERVIRED : CODIGO_MULTIRED;
  ...
  const oracleData: Record<string | number, any>[] = (
    rows?.map((row) => {
  ...
  const ingresos = oracleData.reduce(
    (acc, item) => acc + Number(item.ingresos || 0),
    0,
  );
  const egresos = oracleData.reduce(
    (acc, item) => acc + Number(item.egresos || 0),
    0,
  );
  const abonos = oracleData.reduce(
    (acc, item) => acc + Number(item.abonos_cartera || 0),
    0,
  );
  const saldoInicial = Number(CarteraInicial?.SALDO_ANT || 0);
  const saldoFinal =
    ingresos + saldoInicial - egresos - abonos - Number(base?.BASE || 0);
  ...
  return {
    ...
    empresa: SellerPowerBi?.CCOSTO
      ? CCOSTO_LABELS[SellerPowerBi.CCOSTO] || "No permitido"
      : null,
    saldoInicial,
...
export const getReportMngr = async (req: Request, res: Response) => {
  ...
  if (
    !fecha1Raw.match(/^\d{4}-\d{2}-\d{2}$/) ||
    !fecha2Raw.match(/^\d{4}-\d{2}-\d{2}$/)
  ) {
    return res
      .status(400)
  ...
  try {
    ...
    if (!SellerPowerBi) {
      res
        .status(404)
        .json({
          message: "El documento ingresado no se encuentra en BD POWER BI",
        });
      return;
    }
    ...
    console.log(
      `[${requestId}] Ejecutando getReportMngr para vinculado ${vinculado}`,
    );
...
export const getReportMngrWsp = async (req: Request, res: Response) => {
  ...
  if (mode === "upsert-contact") {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ success: true, created, phone: normalizedPhone, contact });
    } catch (error) {
      console.error("Error upserting contact", error);
      return res
        .status(500)
        .json({ message: "No fue posible guardar el teléfono" });
    }
  }
  ...
  try {
    ...
    for (const cartera of allCarteras) {
      ...
      if (!seller || !seller.DOCUMENTO) {
        try {
          const found = await Sellers.findOne({
            where: { DOCUMENTO: String(cartera.VINCULADO) },
          });
      ...
      const phone = await findContactPhone(
        seller?.DOCUMENTO || cartera.VINCULADO,
      );
      const validation = getDispatchValidation({
      ...
      summaries.push({
        ...
        // Mantener compatibilidad con la respuesta detallada: incluir campos clave
        BASE: Number((cartera as any).BASE || 0),
    ...
    const uniqueSummariesByVinculado = summaries.reduce<
      Record<number, BulkSummary>
    >((acc, summary) => {
      ...
      const shouldReplace =
        !existing.isValidForDispatch && summary.isValidForDispatch;
      if (shouldReplace) {
        acc[summary.vinculado] = summary;
      }
      return acc;
    ...
    if (vinculado) {
      ...
      if (specific) {
        return res.status(200).json({
          ...
          Seller: {
            NOMBRES: specific.sellerName,
            DOCUMENTO: specific.documento,
            CCOSTO: specific.ccosto,
            NOMBRECARGO: specific.cargo,
          },
          base: specific.base,
          bulk: false,
    ...
    if (mode === "dispatch") {
      console.log(
        `Dispatch mode requested. selectedVinculados=${JSON.stringify(selectedVinculados)}`,
      );
      if (!selectedVinculados || selectedVinculados.length === 0) {
        return res
          .status(400)
          .json({
            message: "Debe seleccionar al menos una cartera para enviar",
          });
      }
    ...
    return res
      .status(500)
      .json({
        message: "Error al procesar carteras",
        error: (error as Error).message,
      });
  }
};
...
export const getDetalladoWsp = async (req: Request, res: Response) => {
  ...
  try {
    const results = await CarteraDataServices(
      empresa as string,
      abs === "true",
    );
    ...
    const mapped = await Promise.all(
      results.map(async (item: any) => {
        ...
        const contact = await Ifocontacto.findOne({
          where: { DOCUMENTO: documento || String(item.VINCULADO) },
        });
        const phone = normalizePhone(
          contact?.CELULAR || contact?.TELEFONO || undefined,
        );
        ...
        return {
          ...
          cartera: Number(item.SALDO_ANT || 0),
          phone,
          hasContact: validation.isValidForDispatch,
          isValidForDispatch: validation.isValidForDispatch,
          validationReason: validation.validationReason,
